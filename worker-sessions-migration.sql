-- ============================================================
-- Worker Sessions System — Supabase Migration
-- Run this in the Supabase SQL Editor for the TalentLedger project
-- ============================================================
...
create table if not exists workers (
  ...
  name           text    not null,
  email          text    unique,
  employee_id    text    unique,        -- short kiosk ID (e.g. "EMP042")
...
-- ============================================================
-- 2. EVENTS  (created by employer / internal team)
-- ============================================================
create table if not exists events (
  ...
  role            text,                 -- type of worker needed
  company         text,
  location        text,
  status          text    default 'pending',  -- pending | open | in_progress | completed | cancelled
  created_by      text,                 -- name/email of creator
  created_at      timestamptz default now()
);

-- ============================================================
-- 3. EVENT ASSIGNMENTS  (internal team links workers to events)
-- ============================================================
create table if not exists event_assignments (
  ...
  status        text    default 'assigned',  -- assigned | confirmed | completed | no_show
  notes         text,
  assigned_by   text,
  created_at    timestamptz default now(),
...
-- ============================================================
-- 4. WORKER SESSIONS  (actual check-in / check-out records)
-- ============================================================
...
-- ============================================================
-- 5. RPC: worker_get_status
--    Verifies credentials and returns current check-in state.
--    Called by the kiosk BEFORE showing the action button.
-- ============================================================
create or replace function worker_get_status(
  p_identifier text,           -- email OR employee_id
  p_credential text,           -- plaintext password OR PIN
...
begin
  ...
  -- Fetch event name if session is linked to one
...
-- ============================================================
-- 6. RPC: worker_check_in_out
--    Verifies credentials and performs check-in or check-out.
--    Called when the worker clicks the action button.
-- ============================================================
create or replace function worker_check_in_out(
  p_identifier text,
  p_credential text,
  p_is_pin     boolean default false,
  p_event_id   uuid    default null      -- optional: link to a specific event
)
returns json
language plpgsql
security definer
...
begin
  ...
  if found then
    ...
    -- ── CHECK IN ──
    -- Find assignment if event_id provided
    ...
    -- Update event status if first worker checking in
    if p_event_id is not null then
...
-- Authenticated TalentLedger users can read/write everything
...
-- ============================================================
-- 8. RPC: hash_worker_credentials
--    Called by the internal dashboard when creating a worker.
--    Hashes password and/or PIN server-side with bcrypt.
--    Returns { password_hash, pin_hash }.
-- ============================================================
...
-- Only authenticated TalentLedger staff can hash credentials
